...
Функция СформироватьСтруктуруМетаданных(ИмяПланаОбмена = Неопределено) Экспорт
	...
	ТекНомерСтроки = 1;
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 1, 2, Ложь, "Константы", НСтр("fr = 'Constantes'; ru = 'Константы'; en = 'Constants'; es = 'Constantes'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 3, 4, Истина, "Справочники", НСтр("fr = 'Catalogues'; ru = 'Справочники'; en = 'Reference books'; es = 'Manuales'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 5, 6, Истина, "Последовательности", НСтр("fr = 'Séquences'; ru = 'Последовательности'; en = 'Sequences'; es = 'Secuencias'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 7, 8, Истина, "Документы", НСтр("fr = 'Documents'; ru = 'Документы'; en = 'Documents'; es = 'Documentos'"));
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = НСтр("fr = 'non spécifié'; ru = 'не задано'; en = 'not specified'; es = 'no especificado'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
	...
	ИначеЕсли НРег(Прав(СокрЛП(НастройкаАдресВнешнейОбработкиЗапросов), 4)) = ".epf" Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			Если ЭтоФайловаяБаза() Тогда
				РасположениеФайла = "";
			Иначе
				РасположениеФайла = НСтр("fr = 'sur le serveur'; ru = 'на сервере'; en = 'on the server'; es = 'en el servidor'");
			КонецЕсли;
	...
	Иначе
		...
		Если Метаданные.Обработки.Найти(НастройкаАдресВнешнейОбработкиЗапросов) = Неопределено Тогда
			Текст = НСтр("fr = 'Le traitement de"" & # 160; %1 & # 160; "" introuvable dans la configuration'; ru = 'Обработка ""%1"" не найдена в составе конфигурации'; en = 'The ""%1"" treatment was not found in the configuration'; es = 'El procesamiento ""%1 "" no se encontró en la configuración'");
