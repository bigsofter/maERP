...
Процедура ИзменитьРасписаниеНажатие(Элемент)
	Оп = Новый ОписаниеОповещения("ПослеЗакрытияДиалогаРасписания", ЭтотОбъект);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		МасПользователи = ПользователиИнформационнойБазы.ПолучитьПользователей();
	Исключение
		Сообщить("Ошибка при получении списка пользователей информационной базы: " + ОписаниеОшибки());
		МасПользователи = Неопределено;
	КонецПопытки;
	
	Если МасПользователи <> Неопределено Тогда
		
		Для Каждого Пользователь Из МасПользователи Цикл
	...
	РегламентноеЗадание = Обработки.РегламентныеИФоновыеЗадания.Создать().ПолучитьОбъектРегламентногоЗадания(РегламентноеЗаданиеИД);
...
Функция ПолучитьРасписаниеРегламентногоЗадания(УникальныйНомерЗадания) Экспорт
	ОбъектЗадания = Обработки.РегламентныеИФоновыеЗадания.Создать().ПолучитьОбъектРегламентногоЗадания(УникальныйНомерЗадания);
...
&НаКлиенте
Процедура ПослеЗакрытияДиалогаРасписания(РасписаниеИтог, ДополнительныеПараметры) Экспорт
	Если РасписаниеИтог <> Неопределено	Тогда
		Расписание = РасписаниеИтог;
		Элементы.НадписьРасписание.Заголовок = "Выполнять: " + Строка(Расписание);
	КонецЕсли;	
КонецПроцедуры