	...
	Для Каждого Операция Из Параметры.ИзменяемыеРеквизиты Цикл
		...
		ИначеЕсли Операция.ВидОперации = 2 Тогда // изменение дополнительного реквизита
			...
			Если ЗначениеЗаполнено(Значение) Тогда
				...
				ОписаниеСвойства = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Операция.Свойство, "ТипЗначения, МногострочноеПолеВвода");
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(
			НСтр("fr = 'Le format du paramètre de ligne de version1 est incorrect & # 160;: %1'; ru = 'Неправильный формат параметра СтрокаВерсии1: %1'; en = 'Incorrect format of the Version String1 parameter: %1'; es = 'Formato incorrecto del parámetro de string Versión1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(
			НСтр("fr = 'Le format du paramètre de ligne version2 est incorrect & # 160;: %1'; ru = 'Неправильный формат параметра СтрокаВерсии2: %1'; en = 'Incorrect format of the StringVersion parameter2: %1'; es = 'Formato de parámetro de cadena incorrecto: % 1'"), СтрокаВерсии2);
	КонецЕсли;
