...
Процедура ПривестиСуммыПозицийЧекаКСуммеТаблицыОплат(ОсновныеПараметры, ОбщаяСуммаЧека, СуммаПолнойОплаты, СуммаПередачиБезОплаты, Отказ, ОписаниеОшибки) Экспорт
	...
	Если СуммаФактическихОплат < 0 Тогда
		Отказ = Истина;
		ОписаниеОшибки = НСтр("ru = 'Сумма по строкам с указанием признака полной оплаты меньше чем сумма самих оплат'; fr = 'Le montant sur les lignes indiquant le signe du paiement intégral est inférieur au montant des paiements eux-mêmes'; en = 'The amount along the lines indicating the indication of full payment is less than the amount of the payments themselves'; es = 'La cantidad por línea con indicación del signo de pago completo es menor que la cantidad de los pagos en sí'") ;
		Возврат;
	ИначеЕсли СуммаФактическихОплат = 0 Тогда
		Отказ = Истина;
		ОписаниеОшибки = НСтр("ru = 'Необходимо указать оплату'; fr = 'Vous devez spécifier le paiement'; en = 'You must specify the payment'; es = 'Es necesario especificar el pago'") ;
		Возврат;
	КонецЕсли;
