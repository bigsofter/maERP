...
Функция ПредставлениеНоменклатуры(Номенклатура, Характеристика, Серия = Неопределено) Экспорт
	Результат = СокрЛП(Номенклатура);
	ДобавитьКСтроке(Результат, СокрЛП(ОбщегоНазначенияВызовСервера.ЗначениеРеквизитаОбъекта(Номенклатура,"Артикул")), ", ");	
	Если ЗначениеЗаполнено(Характеристика) Тогда
		ДобавитьКСтроке(Результат, СокрЛП(Характеристика), ", ");	
	КонецЕсли;	
	Если ЗначениеЗаполнено(Серия) Тогда
		ДобавитьКСтроке(Результат, СокрЛП(Серия), нСтр("ru = ', серия - '; fr = ', série -'"));	
	КонецЕсли;
	Возврат Результат;	
КонецФункции
