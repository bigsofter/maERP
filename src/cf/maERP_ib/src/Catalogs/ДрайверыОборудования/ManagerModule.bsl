...
Процедура ЗаполнитьПредопределенныйЭлемент(ОбработчикДрайвера, ИдентификаторОбъекта = Неопределено, ИмяМакетаДрайвера = Неопределено, 
	...
	Исключение
		Сообщение = НСтр("ru = 'Предопределенный элемент ""%Параметр%"" не найден.'; fr = 'L''élément prédéfini ""%Параметр%"" n''a pas été trouvé.'; en = 'Predefined element ""%Параметр%"" parameter was not found.'; es = 'No se encontró el elemento predefinido ""%Параметр%"".'");
