...
//  ОписаниеСтроки - Строка, Неопределено - описание из строки документа (коммерческое предложение): для печатной
//              формы выводится вместо описания из карточки, независимо от флага ВыводитьНаПечатьОписание.
//
// Возвращаемое значение:
//  Строка - представление.
//
Функция ПредставлениеНоменклатуры(Номенклатура, Характеристика, Серия = Неопределено, ЭтоПечать = Ложь,
	НоменклатураСтрока = "", ОписаниеСтроки = Неопределено) Экспорт
	...
	Если ОписаниеСтроки <> Неопределено Тогда
		Если ЗначениеЗаполнено(ОписаниеСтроки) Тогда
			Результат = Результат + Символы.ПС + СокрЛП(ОписаниеСтроки);
		КонецЕсли;
	ИначеЕсли Реквизиты.ВыводитьНаПечатьОписание = Истина И ЗначениеЗаполнено(Реквизиты.Описание) Тогда
...
// Номер документа в виде для печати и заголовков: без префикса и лидирующих нулей -
// как НомерНаПечать в УНФ. "АСФР-000015" -> "15", "00000000002" -> "2".
//
// Параметры:
//  Номер - Строка - номер документа как он хранится
//
// Возвращаемое значение:
//  Строка - нормализованный номер; исходный номер, если цифр в нём нет
//
Функция НомерНаПечать(Знач Номер) Экспорт

	Номер = СокрЛП(Номер);

	// Префикс - всё до первой цифры (буквы информационной базы, организации, разделители).
	Для Позиция = 1 По СтрДлина(Номер) Цикл
		Если СтрНайти("0123456789", Сред(Номер, Позиция, 1)) > 0 Тогда
			Возврат УбратьЛидирующиеНули(Сред(Номер, Позиция));
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьТипЗначенияПоля(Элемент, Реквизит, ТипЗначенияСвойства) Экспорт
	...
	Иначе
		// Тип значения у вида свойства не задан: ограничивать поле нечем, оставляем как есть.
		// Без этой ветки форма значения свойства падала на существующем элементе, у владельца
		// которого тип не выбран (найдено прогоном форм на образцах 2026-08-31).
		Возврат;
	КонецЕсли;
