...
// Возвращает значения реквизитов нескольких объектов одного типа одним запросом.
//
// Параметры:
//  Ссылки - Массив, ФиксированныйМассив - ссылки на объекты одного типа (пустые и Неопределено пропускаются).
//  Реквизиты - Строка, Массив, Структура - см. ЗначенияРеквизитовОбъекта.
//
// Возвращаемое значение:
//  Соответствие - ключ: ссылка, значение: Структура реквизитов (см. ЗначенияРеквизитовОбъекта).
//                 Для ссылок, которых нет в базе, элемента в соответствии нет.
//
Функция ЗначенияРеквизитовОбъектов(Ссылки, Знач Реквизиты) Экспорт
	
	Результат = Новый Соответствие;
	СтруктураРеквизитов = СтруктураЗапрашиваемыхРеквизитов(Реквизиты);
	МассивСсылок = Новый Массив;
	Для Каждого Ссылка Из Ссылки Цикл
		Если ЗначениеЗаполнено(Ссылка) И МассивСсылок.Найти(Ссылка) = Неопределено Тогда
			МассивСсылок.Добавить(Ссылка);
		КонецЕсли;
	КонецЦикла;
	Если СтруктураРеквизитов.Количество() = 0 Или МассивСсылок.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПсевдонимЗаданнойТаблицы.Ссылка КАК Ссылка" + ТекстПолей + "
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого КлючИЗначение Из СтруктураРеквизитов Цикл
			Значения.Вставить(КлючИЗначение.Ключ, Выборка[КлючИЗначение.Ключ]);
		КонецЦикла;
		Результат.Вставить(Выборка.Ссылка, Значения);
	КонецЦикла;
