...
Функция ПустыеПараметрыКомпоновкиОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ПоСубсчетам", Истина); // детализация до субсчетов
	ПараметрыОтчета.Вставить("ПоСубсчетамКорСчетов", Истина); // детализация до субсчетов
...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Возврат СтрШаблон(
			НСтр("fr = 'Analyse du compte %1%2'; ru = 'Анализ счета %1%2'; en = 'Account analysis %1%2'; es = 'Análisis de cuenta %1%2'"),
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ГруппаДт.Заголовок     = НСтр("fr = 'Débit'; ru = 'Дебет'; es = 'Débito'");
	...
	ГруппаКт.Заголовок     = НСтр("fr = 'Crédit'; ru = 'Кредит'; en = 'Credit'; es = 'Haber'");
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Для Каждого Макет Из МакетКомпоновки.Макеты Цикл
		...
		Если МакетыЗаголовков.Найти(Макет) <> Неопределено Тогда
			...
			НовыйЭлемент.Значение = НСтр("fr = ""Solde d'ouverture""; ru = 'Начальное сальдо'; en = 'Initial balance'; es = 'Saldo de apertura'");
		...
		Если МакетыПодвалов.Найти(Макет) <> Неопределено Тогда
			...
			Для Каждого ПараметрМакета Из Макет.Параметры Цикл
				Если ТипЗнч(ПараметрМакета) = Тип("ПараметрОбластиРасшифровкаКомпоновкиДанных") Тогда
					УдаляемыеВыражения = Новый Массив;
					Для Каждого ВыражениеПоля Из ПараметрМакета.ВыраженияПолей Цикл
						Если ВыражениеПоля.Поле = "БухТипРесурса" Тогда
							УдаляемыеВыражения.Добавить(ВыражениеПоля);
						КонецЕсли;
					КонецЦикла;
					Для Каждого ВыражениеПоля Из УдаляемыеВыражения Цикл
			...
			НовыйЭлемент.Значение = НСтр("fr = 'Mouvement'; ru = 'Оборот'; en = 'Turnover'; es = 'Revolución'");
			...
			НовыйЭлемент.Значение = НСтр("fr = 'Solde de clôture'; ru = 'Конечное сальдо'; en = 'Final balance'; es = 'Saldo de cierre'");
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.ШаблонПредставления = НСтр("fr = 'Rapport sur les ecritures'; ru = 'Отчет по проводкам'; en = 'Transaction Report'; es = 'Informe de transacciones'");
...
Процедура ДобавитьУсловиеПоВыбраннымРесурсам(ПараметрыОтчета, Схема)
	...
	Для Каждого ИмяПоказателя Из ПолучитьНаборПоказателей() Цикл
		...
		Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда
			...
			ИначеЕсли ИмяПоказателя = "ВалютнаяСумма"
			 Или ИмяПоказателя = "Количество"
			 Или ИмяПоказателя = "Контроль"
			 Или ИмяПоказателя = "СверкаНУ" Тогда
