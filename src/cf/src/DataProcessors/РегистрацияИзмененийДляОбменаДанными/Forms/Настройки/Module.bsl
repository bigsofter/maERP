...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийОбъект.ЭтоФайловаяБаза() Тогда
		...
		Если ТекЭлемент <> Неопределено Тогда
			ТекЭлемент.Представление = НСтр("fr = 'Dans le catalogue:'; ru = 'В каталоге:'; en = 'In the catalog:'; es = 'En el catálogo:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаЗапросаПутьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.Фильтр = НСтр("fr = 'Traitements externes (*.epf)|*.epf'; ru = 'Внешние обработки (*.epf)|*.epf'; en = 'External processing (*.epf)|*.epf'; es = 'Tratamientos externos (*.epf)|*.epf'");
...
Процедура СообщитьОбОшибке(Текст, ИмяРеквизита = Неопределено)
	...
	Если ИмяРеквизита = Неопределено Тогда
		ЗаголовокОшибки = НСтр("fr = 'Erreur'; ru = 'Ошибка'; en = 'Mistake'; es = 'Error'");
