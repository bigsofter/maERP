...
Процедура РедактироватьОбъект(Команда)
	...
	ПараметрыФормы.Вставить("мОбъектСсылка", Элементы.ДеревоУзловОбмена.ТекущиеДанные.Ссылка);
...
Функция ВыбранныеУзлы(НовыеДанные = Неопределено)
	...
	Если НовыеДанные <> Неопределено Тогда
		...
		ВнутрУстановитьВыбранныеУзлы(ЭтотОбъектОбработки(), ДеревоУзловОбмена, НовыеДанные, Отмеченные);
		Возврат Отмеченные;
	КонецЕсли;
...
Процедура ПрочитатьДеревоУзловОбмена()
	Дерево = ЭтотОбъектОбработки().СформироватьДеревоУзлов();
