...
//				ОткрытиеСУзломПараметром = Истина;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(
			НСтр("fr = 'Commande non reconnue "" & # 160; %1 & # 160;""'; ru = 'Нераспознанная команда ""%1""'; en = 'The unrecognized ""%1"" command'; es = 'Comando""%1 "" no reconocido'"), "%1", Параметры.ИдентификаторКоманды);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("fr = ""Enregistrer le résultat de la requête"
"sur le nœud '%1'?""; ru = 'Зарегистрировать результат запроса"
"на узле ""%1""?'; en = 'Register the query result"
"on the ""%1"" node?'; es = 'Registrar el resultado de la consulta"
"¿en el nodo ""%1""?'");
	Иначе
		Текст = НСтр("fr = ""Annuler l'enregistrement du résultat de la requête"
"sur le nœud '%1'?""; ru = 'Отменить регистрацию результата запроса"
"на узле ""%1""?'; en = 'Should I cancel the registration of the query result"
"on the %1 node?'; es = 'Cancelar el registro del resultado de la consulta"
"¿en el nodo ""%1""?'");
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", Строка(УзелОбменаСсылка));

	ЗаголовокВопроса = НСтр("fr = 'Confirmation'; ru = 'Подтверждение'; en = 'Confirmation'; es = 'Confirmación'");
...
Процедура РедактироватьСсылку(Команда)
	...
	ПараметрыФормы.Вставить("мОбъектСсылка", ТекДанные.Ссылка);
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("fr = 'Numéro envoyé'; ru = 'Номер отправленного'; en = 'The number of the sent'; es = 'Número enviado'");
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("fr = 'N° de message : envoyé %1, reçu %2'; ru = '№ сообщений: отпр. %1, прин. %2'; en = ' Message number: sent %1, taken %2'; es = ' número de mensajes: enviado %1, tomado %2'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("fr = 'Enregistrement des modifications:'; ru = 'Регистрация изменений:'; en = 'Registration of changes:'; es = 'Registrar cambios:'");
			ТекстПредупреждения = НСтр("fr = '%1 modifications enregistrées à partir de %2"
"sur le nœud %0'; ru = 'Зарегистрировано %1 изменений из %2"
"на узле ""%0""'; en = 'Registered %1 of %2 changes"
"on node ""%0""'; es = '%1 cambios registrados de %2"
"en el nodo ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("fr = 'Désinscription:'; ru = 'Отмена регистрации:'; en = 'Cancellation of registration:'; es = 'Cancelación del registro:'");
			ТекстПредупреждения = НСтр("fr = 'Annulation de l''enregistrement de % 1 modifications "
"sur le nœud ""%0"".'; ru = 'Отменена регистрация %1 изменений "
"на узле ""%0"".'; en = 'Registration of %1 changes"
"on node ""%0"" has been canceled.'; es = 'Se canceló el registro de % 1 cambios "
"en el nodo ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("fr = 'Modification du numéro de message:'; ru = 'Изменение номера сообщения:'; en = 'Changing the message number:'; es = 'Cambiar el número de mensaje:'");
		ТекстПредупреждения = НСтр("fr = 'Le numéro de message a été remplacé par %3"
"%1 objet (s)'; ru = 'Номер сообщения изменен на %3"
"у %1 объекта(ов)'; en = 'Message number changed to %3"
"for %1 of the object(s)'; es = 'Número de mensaje cambiado a %3"
"%1 objeto(s)'");
	КонецЕсли;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	ИначеЕсли Колво = 1 Тогда
		Текст = НСтр("fr = ""Désinscrire & # 160;: %2"
"sur le nœud '%1'?""; ru = 'Отменить регистрацию ""%2"""
"на узле ""%1""?'; en = 'Cancel registration of ""%2"""
"on node ""%1""?'; es = 'Cancelar registro ""%2"""
"¿en el nodo ""%1""?'");
	Иначе
		Текст = НСтр("fr = ""Désinscrire les constantes sélectionnées"
"sur le nœud '%1'?""; ru = 'Отменить регистрацию выбранных констант"
"на узле ""%1""?'; en = 'To cancel registration of the selected constants"
"on the node ""%1""?'; es = 'Anular el registro de las constantes seleccionadas"
"¿en el nodo ""%1""?'");
	КонецЕсли;
...
"¿en el nodo ""%1""?'");
	Иначе
		Текст = НСтр("fr = ""Désenregistrer les objets sélectionnés"
"sur le nœud '%1'?""; ru = 'Отменить регистрацию выбранных объектов"
"на узле ""%1""?'; en = 'Cancel registration of selected objects"
"on the node ""%1""?'; es = 'Anular el registro de los objetos seleccionados"
"¿en el nodo ""%1""?'");
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("fr = 'Voulez-vous enregistrer %1 pour le déchargement sur l''hôte"" & # 160; %2 & # 160;""?"
""
"La modification de l''enregistrement d''un grand nombre d''objets peut prendre beaucoup de temps!'; ru = 'Зарегистрировать %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время!'; en = 'Register %1 for upload on node ""%2""?"
""
"Changing the registration of a large number of objects can take a long time!'; es = '¿Registrar %1 para descargar en el nodo ""%2""?"
""
"¡Cambiar el registro de un gran número de objetos puede llevar mucho tiempo!'");
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("fr = 'Voulez-vous annuler l''inscription de %1 pour le déchargement sur l''hôte"" & # 160; %2 & # 160;""?"
""
"La modification de l''enregistrement d''un grand nombre d''objets peut prendre beaucoup de temps!'; ru = 'Отменить регистрацию %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время!'; en = 'Cancel registration of %1 for unloading on node ""%2""?"
""
"Changing the registration of a large number of objects can take a long time!'; es = '¿Anular el registro de %1 para descargar en el nodo ""%2""?"
""
"¡Cambiar el registro de un gran número de objetos puede llevar mucho tiempo!'");
...
"sur le nœud '%1'?""; ru = 'Отменить регистрацию выбранных объектов"
"на узле ""%1?'; en = 'Cancel registration of selected objects"
"on the %1 node?'; es = 'Anular el registro de los objetos seleccionados"
"en el nodo "" %1?'");
	Иначе
		Текст = НСтр("fr = ""Désinscrire & # 160;: %2"
"sur le nœud '%1'?""; ru = 'Отменить регистрацию ""%2"""
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	ТекстВопроса = НСтр("fr = ""Aucun traitement n'est spécifié dans les paramètres pour exécuter les requêtes."
"Configurer maintenant?""; ru = 'В настройках не указана обработка для выполнения запросов."
"Настроить сейчас?'; en = 'The processing for executing requests is not specified in the settings."
"Set it up now?'; es = 'La configuración no especifica el procesamiento para ejecutar consultas."
"Configurar ahora?'");

	ЗаголовокВопроса = НСтр("fr = 'Réglages'; ru = 'Настройки'; en = 'Settings'; es = 'Ajustes'");
...
""
"%2"
"Pour enregistrer ou désinscrire une communication sur un hôte"
"""%3"""
"sélectionnez le type d''objet à gauche dans l''arborescence des métadonnées et utilisez"
"commandes Enregistrer ou Désinscrire'; ru = '%1."
""
"%2"
...
""
"%2"
"To register or unregister data exchange on"
"the %3 node"
", select the object type on the left in the metadata tree and use"
"the Register or Unregister commands'; es = '%1."
""
"%2"
"Para registrar o anular el registro de un intercambio de datos en un host"
"""%3"""
"seleccione el tipo de objeto a la izquierda en el árbol de metadatos y"
"los comandos ""Registrar ""o""Cancelar registro""'");
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено,
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("fr = 'tous les objets %1 dans la hiérarchie de vue sélectionnée'; ru = 'все объекты %1 по выбранной иерархии вида'; en = 'all %1 objects in the selected view hierarchy'; es = 'todos los objetos %1 en la jerarquía de vista seleccionada'");
