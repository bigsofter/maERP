...
Процедура ПослеВыбораОтчета(Результат, ДопПараметры) Экспорт
	...
	Если Результат.Значение = "EtatZ" Тогда 
		Если ЗначениеЗаполнено(КассоваяСмена) Тогда
			ПечатныеФормыКлиент.ВыполнитьКомандуПечатиНаПринтер("Документ.КассоваяСмена", "EtatZ",
				КассоваяСмена, ОбщегоНазначенияВызовСервера.ПолучитьИмяПринтераЧековНаСервере());
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПечататьЧек(ИмяМенеджераПечати, Док)

	Если ВыводитьЧекНаЭкрнПередПечатью Тогда
		ПечатныеФормыКлиент.ВыполнитьКомандуПечати(ИмяМенеджераПечати, "Чек", Док, ЭтотОбъект);
	Иначе
		ПечатныеФормыКлиент.ВыполнитьКомандуПечатиНаПринтер(ИмяМенеджераПечати, "Чек", Док,
...
&НаКлиенте
Процедура ПечататьНакладную(Док)

	// В режиме ПДВ накладная печатается без цен.
	ПараметрыПечати = Новый Структура("БезЦен", ЭтоPDV);
	ПечатныеФормыКлиент.ВыполнитьКомандуПечати("Документ.РеализацияТоваровУслуг", "DevisBL",
		Док, ЭтотОбъект, ПараметрыПечати);
