...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Команда.ИмяМакета = "ОтгрузкаТовара";
	Команда.Представление = НСтр("ru = 'Отгрузка товара'; fr = 'Bon de livraison';
		|en = 'Goods shipment'; es = 'Albarán de salida'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатныеФормы.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтгрузкаТовара") Тогда
		Таб = Новый ТабличныйДокумент;
		Для Каждого Док Из МассивОбъектов Цикл
			НомерСтрокиНачало = Таб.ВысотаТаблицы + 1;
			Документы.РеализацияТоваровУслуг.PrintBonDeLivtaison(Док, Таб);
			ПечатныеФормы.ЗадатьОбластьПечати(Таб, НомерСтрокиНачало, ОбъектыПечати, Док);
		КонецЦикла;
		ПечатныеФормы.ВывестиТабличныйДокумент(КоллекцияПечатныхФорм, "ОтгрузкаТовара",
			НСтр("ru = 'Отгрузка товара'; fr = 'Bon de livraison';
			|en = 'Goods shipment'; es = 'Albarán de salida'"),
			Таб, "Документ.ОтгрузкаТовара.ОтгрузкаТовара");
	КонецЕсли;
