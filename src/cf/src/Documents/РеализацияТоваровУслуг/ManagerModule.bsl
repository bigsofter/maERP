...
Функция ВывестиDevisBL(Док, Spreadsheet, Параметры)
	// Общий вывод Devis / BL / Avoir по РТУ, заказу покупателя и возвратам.
	...
	Области = ОбластиМакетаDevisBL(Template);
	Spreadsheet.РазмерСтраницы = Параметры.РазмерСтраницы;
	Spreadsheet.АвтоМасштаб = Истина;

	Карточки = Новый Соответствие;
	Пока Selection.Следующий() Цикл
		ЗаполнитьШапкуDevisBL(Области.Header, Selection, Док.Дата, Параметры.НазваниеДокумента, Карточки);
		...
		Если Не Параметры.БезЦен Тогда
			...
			ЗаполнитьОплатыВПодвале(Области.FooterTotal, Selection.DocRef, Selection.TotalSomme, Selection.Devise);
		КонецЕсли;
		Spreadsheet.Вывести(Области.FooterTotal);
		ДобитьСтраницуПустымиСтроками(Spreadsheet, Области.EmptyLine, МассивОбластей(Области.Footer));
...
Функция ОбластиМакетаDevisBL(Template)
	Области = Новый Структура;
	Области.Вставить("Header", Template.ПолучитьОбласть("Header"));
	Области.Вставить("TabularSectionHeader", Template.ПолучитьОбласть("TabularSectionHeader"));
	Области.Вставить("TabularSection", Template.ПолучитьОбласть("TabularSection"));
...
Процедура ЗаполнитьШапкуDevisBL(Header, Selection, ДатаДокумента, НазваниеДокумента, Карточки)
	Header.Параметры.Заполнить(Selection);
	Header.Параметры.Организация = Selection.Organisation;
	Header.Параметры.Number = НСтр("ru = 'Номер: '; fr = 'Numéro: '; en = 'Number: '; es = 'Número: '")
		+ РаботаСДокументами.СформироватьЦифровойНомер(Selection.Number) + "/" + Формат(Selection.Date, "Л=fr; ДФ=yy; ДЛФ=DD");
	Header.Параметры.Date = НСтр("ru = 'Дата: '; fr = 'Date: '; en = 'Date: '; es = 'Fecha: '")
		+ Формат(Selection.Date, "Л=fr; ДФ=dd/MM/yyyy; ДЛФ=DD");

	Карточка = КарточкаКонтрагента(Карточки, Selection.Organisation, ДатаДокумента);
	Header.Параметры.Адрес = Карточка.АдресЮридический;
	Header.Параметры.Телефоны = МеткаТелефон() + Карточка.Телефоны;
	Header.Параметры.RC = "RC: " + Карточка.Реквизиты.RC;
	Header.Параметры.Patente = "PATENTE: " + Карточка.Реквизиты.TaxeProfessionnelle;
	Header.Параметры.IF = "IF: " + Карточка.Реквизиты.NIF;
	Header.Параметры.ICE = "ICE: " + Карточка.Реквизиты.ICE;
	Header.Параметры.Site = НСтр("ru = 'Сайт: '; fr = 'Site web: '; en = 'Website: '; es = 'Sitio web: '") + Карточка.Реквизиты.Web;
	Header.Параметры.Email = "E-mail: " + Карточка.Реквизиты.EMail;

	КарточкаКлиента = КарточкаКонтрагента(Карточки, Selection.Client, ДатаДокумента);
	Client = РаботаСДокументами.СформироватьЦифровойНомер(Selection.КодКлиента) + Символы.ПС + Selection.НаименованиеКлиента;
	Client = Client + Символы.ПС + МеткаТелефон() + КарточкаКлиента.Телефоны;
	Header.Параметры.Client = Client;
	Header.Параметры.НазваниеДокумента = НазваниеДокумента;
	Header.Параметры.Комментарий = Selection.Комментарий;
	Header.Параметры.НомерЗаказа = РаботаСДокументами.СформироватьЦифровойНомер(Selection.НомерЗаказа);
КонецПроцедуры
...
Процедура ЗаполнитьОплатыВПодвале(Область, Документ, СуммаДокумента, Валюта)
	// Заполняет в области итогов сумму оплат по документу и остаток к оплате.
	Оплаты = РегистрыНакопления.ВзаиморасчётыСПокупателями.GetTableOfPayments(Документ);
	ВсегоОплачено = Оплаты.Итог("Somme");
	Область.Параметры.TotalSommePaiement = ФорматСуммы(ВсегоОплачено, Валюта);
	Область.Параметры.ОстатокОплаты = ФорматСуммы(СуммаДокумента - ВсегоОплачено, Валюта);
КонецПроцедуры
