...
Процедура ДобитьСтраницуПустымиСтроками(Spreadsheet, ОбластьПустойСтроки, ОбластиВнизу)
	// Выводит пустые строки, пока они вместе с областями подвала помещаются на текущую страницу:
	...
	Пока Предохранитель > 0 И Spreadsheet.ПроверитьВывод(МасОбластей) Цикл
		Spreadsheet.Вывести(ОбластьПустойСтроки);
...
Функция ПараметрыПечатиDevisBL(ИмяМакета, РазмерСтраницы, НазваниеДокумента, БезЦен)
	...
	Параметры.Вставить("ОбластьИтогов", "TabularSectionFooterTotal");
...
Функция ВывестиDevisBL(Док, Spreadsheet, Параметры)
	...
	Пока Selection.Следующий() Цикл
		...
		Spreadsheet.Вывести(Области.Header, Selection.Уровень());
		Spreadsheet.Вывести(Области.TabularSectionHeader);
		ВывестиСтрокиDevisBL(Spreadsheet, Области, Selection, Параметры.БезЦен);
		...
		КонецЕсли;
		Spreadsheet.Вывести(Области.FooterTotal);
		ДобитьСтраницуПустымиСтроками(Spreadsheet, Области.EmptyLine, МассивОбластей(Области.Footer, Области.FooterCodes));
...
Процедура ВывестиСтрокиDevisBL(Spreadsheet, Области, Selection, БезЦен)
	SelectionTabularSection = Selection.Выбрать();
	НомерСтроки = 0;
	МасОбластей = Новый Массив;
	МасОбластей.Добавить(Области.TabularSection);
	...
	Пока SelectionTabularSection.Следующий() Цикл
		...
		Если Не Spreadsheet.ПроверитьВывод(МасОбластей) Тогда
			...
			ДобитьСтраницуПустымиСтроками(Spreadsheet, Области.EmptyLine, МассивОбластей(Области.FooterCodes));
			Spreadsheet.Вывести(Области.FooterCodes);
			Spreadsheet.ВывестиГоризонтальныйРазделительСтраниц();
			Spreadsheet.Вывести(Области.Header, Selection.Уровень());
			Spreadsheet.Вывести(Области.TabularSectionHeader);
