...
Процедура ПрименитьНастройку(Команда)
	...
	Если УдаленныеДвижения.Количество() > 0 Тогда
		...
		Если УдаленныеДвижения.Количество() = 1 Тогда
			ТекстВопроса = НСтр("fr = 'Les mouvements de registre ""%ИмяРегистра%""sont supprimés. "
"Continuer?'; ru = 'Будут удалены движения регистра ""%ИмяРегистра%"". "
"Продолжить?'; en = 'Movements of the ""%ИмяРегистра%"" register will be deleted. "
"Continue?'; es = 'Se eliminarán los movimientos del registro "" %ИмяРегистра%"". "
"¿Continuar?'");
			ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ИмяРегистра%", УдаленныеДвижения[0]);
		Иначе
			ТекстВопроса = НСтр("fr = 'Les mouvements des registres %СписокРегистров% sont supprimés. "
"Continuer?'; ru = 'Будут удалены движения регистров %СписокРегистров%. "
"Продолжить?'; en = 'Register movements %СписокРегистров% will be deleted. "
"Continue?'; es = 'Se eliminarán los movimientos de los registros %СписокРегистров%. "
"¿Continuar?'");
