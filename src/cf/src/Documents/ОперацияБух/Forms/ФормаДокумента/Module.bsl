...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Отбора по функциональным опциям нет: механизма БСП, который его делал, в maERP не перенесли,
	// а регистратором хозрасчётного регистра может быть только уже доступный документ.
	...
	// В maERP валюта учёта одна: отдельных констант валют управленческого учёта
	// и финансовой отчётности нет, поэтому все три валюты совпадают.
	ВалютаРУ = Константы.ВалютаРегламентированногоУчета.Получить();
	ВалютаУУ = ВалютаРУ;
	ВалютаФО = ВалютаРУ;
	
	// Учёт по направлениям деятельности, управленческий учёт на хозрасчётном плане счетов и
	// учёт в валюте финансовой отчётности в maERP не перенесены: одноимённых функциональных
	// опций в конфигурации нет, признаки всегда выключены (docs/TECHDEBT.md).
	ИспользуетсяУчетПоНаправлениям = Ложь;
	ВестиУУНаПланеСчетовХозрасчетный = Ложь;
	ВестиУчетНаПланеСчетовХозрасчетныйВВалютеФинОтчетности = Ложь;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// Подсистемы БСП «Управление доступом» в maERP нет, хук убран.
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// Подсистемы БСП «Подключаемые команды» в maERP нет, хук убран.
...
Функция ПараметрыВыбораСубконто(Форма, ТекущиеДанные, ШаблонИмяПоляОбъекта, ИмяПоляСчетУчета)
	...
	Для Индекс = 1 По 3 Цикл
		...
		ИначеЕсли ТипЗнч(ТекущиеДанные[ИмяПоля]) = Тип("СправочникСсылка.МестаХранения") Тогда
...
Процедура ПолучитьСостояниеОтраженияДокумента()
	...
	// Механизм отражения в реглучете (БП) в maERP не перенесен: перечисления
	// СтатусыОтраженияДокументовВРеглУчете в конфигурации нет, статус не вычисляется.
	// Состояние определяется по наличию проводок документа.
	...
	Для каждого СтрокаТЧ Из СторнируемыеДокументы Цикл
		...
		Если НЕ ЗначениеЗаполнено(Документ) Тогда
			ШаблонСообщения = НСтр("ru = 'Не заполнена колонка «%1» в строке %2 списка «%3»';
				|fr = 'La colonne « %1 » n''est pas renseignée dans la ligne %2 de la liste « %3 »';
				|en = 'The ""%1"" column is not filled in row %2 of the ""%3"" list';
				|es = 'La columna «%1» no está rellenada en la línea %2 de la lista «%3»'");
...
Процедура ПересчитатьСуммыПоСтрокеНаСервере(Проводка, Знач Дата, Знач Источник)
	...
	Если ВестиУчетНаПланеСчетовХозрасчетныйВВалютеФинОтчетности
...
Процедура НастроитьОтображениеТаблицыПроводок()
	...
	// Часть колонок бухгалтерского контура платформа на форму не выводит: у них нет пути
	// к данным - реквизитов этих механизмов (направления деятельности, суммы УУ и ФО,
	// налоговые суммы) в maERP нет, docs/TECHDEBT.md. Поэтому все колонки таблицы проводок
	// настраиваются через поиск по имени: чего нет - то и не настраиваем.
	СвойстваНаправления = Новый Структура("Доступность, ОтображатьВШапке",
		ИспользуетсяУчетПоНаправлениям, Шапка3Эатажа);
	НастроитьКолонкуПроводок("ХозрасчетныйНаправлениеДт", СвойстваНаправления);
	НастроитьКолонкуПроводок("ХозрасчетныйНаправлениеКт", СвойстваНаправления);
	
	ЗаголовокБУ = СтрШаблон(НСтр("fr = 'Montant (%1)'; ru = 'Сумма БУ (%1)';
		|en = 'BU amount (%1)'; es = 'Suma de BU (%1)'"), ВалютаРУ);
	УстановитьЗаголовокКолонки("ХозрасчетныйСуммы", ЗаголовокБУ);

	ЗаголовокНУДт = СтрШаблон(НСтр("fr = 'Montant Dt (%1)'; ru = 'Сумма Дт (%1)';
		|en = 'Dt Amount (%1)'; es = 'Suma De DT (%1)'"), ВалютаРУ);
	УстановитьЗаголовокКолонки("ХозрасчетныйСуммаНУДт", ЗаголовокНУДт);

	ЗаголовокНУКт = СтрШаблон(НСтр("fr = 'Montant Ct (%1)'; ru = 'Сумма Кт (%1)';
		|en = 'Ct Amount (%1)'; es = 'Cantidad De CT (%1)'"), ВалютаРУ);
	УстановитьЗаголовокКолонки("ХозрасчетныйСуммаНУКт", ЗаголовокНУКт);
	
	ЗаголовокУУ = СтрШаблон(НСтр("fr = 'Montant (%1)'; ru = 'Сумма УУ (%1)';
		|en = 'Amount of CU (%1)'; es = 'Suma YU (%1)'"), ВалютаУУ);
	УстановитьЗаголовокКолонки("ХозрасчетныйСуммаУУ", ЗаголовокУУ);

	ЗаголовокФО = СтрШаблон(НСтр("fr = 'Montant (%1)'; ru = 'Сумма ФО (%1)';
		|en = 'FO amount (%1)'; es = 'Suma de FO (%1)'"), ВалютаФО);
	УстановитьЗаголовокКолонки("ХозрасчетныйСуммаФО", ЗаголовокФО);
	
	// Настройка полей постоянных и временных разниц (БухгалтерскийУчет.НастроитьПоляУчетаРазниц)
	// в maERP не перенесена - налоговый учёт разниц отсутствует (docs/TECHDEBT.md).
	
КонецПроцедуры

// Устанавливает заголовок колонки таблицы проводок, если она есть на форме.
//
// Параметры:
//  ИмяЭлемента - Строка - имя элемента формы
//  Заголовок   - Строка - новый заголовок колонки
//
&НаСервере
Процедура УстановитьЗаголовокКолонки(ИмяЭлемента, Заголовок)

	НастроитьКолонкуПроводок(ИмяЭлемента, Новый Структура("Заголовок", Заголовок));

КонецПроцедуры

// Настраивает колонку таблицы проводок, если она есть на форме.
//
// Параметры:
//  ИмяЭлемента - Строка    - имя элемента формы
//  Свойства    - Структура - устанавливаемые свойства элемента
//
&НаСервере
Процедура НастроитьКолонкуПроводок(ИмяЭлемента, Свойства)
