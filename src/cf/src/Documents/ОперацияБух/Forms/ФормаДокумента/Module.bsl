...
Процедура НастроитьОтображениеТаблицыПроводок()
	...
	// Колонки направлений деятельности и сумм УУ/ФО платформа на форму не выводит: у них
	// нет пути к данным - реквизитов этих механизмов в maERP нет (docs/TECHDEBT.md).
	// Поэтому настраиваем только те колонки, которые на форме действительно есть.
	...
	НастроитьКолонкуПроводок("ХозрасчетныйСуммаУУ", Новый Структура("Заголовок", ЗаголовокУУ));

	ЗаголовокФО = СтрШаблон(НСтр("fr = 'Montant (%1)'; ru = 'Сумма ФО (%1)';
		|en = 'FO amount (%1)'; es = 'Suma de FO (%1)'"), ВалютаФО);
	НастроитьКолонкуПроводок("ХозрасчетныйСуммаФО", Новый Структура("Заголовок", ЗаголовокФО));
