...
Один запрос `unica.apply` над пакетом несёт упорядоченный массив `ops` и
публикует его атомарно: операции применяются по порядку, каждая видит результат
предыдущих, а отказ любой из них не оставляет частичной записи. Связное
изменение — новый тип и его свойства — поэтому ведётся одним запросом с общим
preview, а не последовательностью отдельных вызовов.
