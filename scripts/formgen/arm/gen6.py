#!/usr/bin/env python3
"""Форма рабочего места производства, редакция 6 (E12.3b: закладка «Переработка» и кнопка «Поступление из переработки»);
редакция 5 (E12.3a: закладки «Выпуск» и «Отгрузки», кнопка «Выпуск по заказу», общие
команды и обработчики строк документов нижних закладок); редакция 4 (E12.2: закладки «Заказы поставщику» и «Поступления»); редакция 3 (замечания владельца к 2.0.15.30): без колонок процентов (проценты - в
...
T.update({
    ...
    'Отгрузки': ('Отгрузки', 'Expéditions', 'Shipments', 'Envíos'),
    'Склад': ('Склад', 'Entrepôt', 'Warehouse', 'Almacén'),
    'Автовыпуск': ('Автовыпуск', 'Production automatique', 'Automatic output', 'Producción automática'),
    'Переработка': ('Переработка', 'Sous-traitance', 'Subcontracting', 'Subcontratación'),
    'Переработчик': ('Переработчик', 'Sous-traitant', 'Processor', 'Procesador'),
    'Поступления из переработки': ('Поступления из переработки', 'Réceptions de sous-traitance', 'Subcontracting receipts',
                                   'Recepciones de subcontratación'),
})
TT = lambda k: T[k]
PURCHASE_STATUS = ['СтатусВРаботу', 'СтатусВЧерновик', 'СтатусОтменить']
PURCHASE_CMDS = {
    'СтатусВРаботу': ('ДокументыСтрокПровести', ('Провести', 'Valider', 'Post', 'Contabilizar'),
    ...
    'СтатусВЧерновик': ('ДокументыСтрокВЧерновик', ('В черновик', 'En brouillon', 'To draft', 'A borrador'),
    ...
    'СтатусОтменить': ('ДокументыСтрокОтменить', ('Отменить', 'Annuler', 'Cancel', 'Cancelar'),
...
CREATE = gen2.CREATE[:2] + ['СоздатьВыпуск'] + gen2.CREATE[2:]
NEW_CREATE = ['СоздатьВыпуск']
...
def build():
    ...
    def purchase_bar(prefix, extra=()):
        ...
        return ([lambda ind: popup(prefix + 'ПодменюСтатус', TT('Статус'), [cmd_button(n) for n in PURCHASE_STATUS], ind)]
                + [cmd_button(n) for n in extra])

    def processing_cols():
        P = 'Переработки'
        return [
            L(P + 'Статус', P + '.Статус', TT('Статус'), read_only=True),
            F(P + 'Поставщик', P + '.Поставщик', TT('Переработчик'), hs=[('OnChange', 'ЗакупкиПоставщикПриИзменении')]),
            ...
            L(P + 'Склад', P + '.Склад', TT('Склад'), read_only=True),
            L(P + 'Поступления', P + '.Поступления', TT('Поступления из переработки'), read_only=True),
            F(P + 'ДокументПредставление', P + '.ДокументПредставление', TT('Документ'), choice_button=True,
              text_edit=False, hs=[('StartChoice', 'СтрокиДокументовДокументПредставлениеНачалоВыбора')]),
        ]
    ...
    def purchase_cols(prefix, receipts):
        ...
        cols += [
            ...
            F(P + 'КоличествоУпаковок', P + '.КоличествоУпаковок', TT('Количество')),
            F(P + 'Упаковка', P + '.Упаковка', TT('Упаковка')),
            F(P + 'Цена', P + '.Цена', TT('Цена'), hs=[('OnChange', 'СтрокиДокументовЦенаПриИзменении')]),
    ...
    def output_cols(prefix, shipments):
        ...
        cols = [
            L(P + 'Статус', P + '.Статус', TT('Статус'), read_only=True),
            F(P + 'Продукция', P + '.Продукция', TT('Продукция'),
        ...
        if shipments:
            cols += [
                F(P + 'Цена', P + '.Цена', TT('Цена'), hs=[('OnChange', 'СтрокиДокументовЦенаПриИзменении')]),
                L(P + 'Сумма', P + '.Сумма', TT('Сумма'), read_only=True),
                L(P + 'Автовыпуск', P + '.Автовыпуск', TT('Автовыпуск'), read_only=True),
            ]
        else:
            cols += [
                L(P + 'Склад', P + '.Склад', TT('Склад'), read_only=True),
                L(P + 'Через', P + '.Через', TT('Через'), read_only=True),
            ]
        cols += [F(P + 'ДокументПредставление', P + '.ДокументПредставление', TT('Документ'), choice_button=True,
                   text_edit=False, hs=[('StartChoice', 'СтрокиДокументовДокументПредставлениеНачалоВыбора')])]
        return cols
    ...
    cmds = [
        ...
        ('СоздатьВыпуск', ('Выпуск по заказу', 'Production par commande', 'Output by order', 'Producción por pedido'),
         ('Выпуск продукции, которую осталось выпустить по заказу на производство', 'Production des produits restant à produire pour la commande en production', 'Output of the products still to be produced for the production order', 'Producción de los productos que faltan por producir del pedido para producción')),
    ...
    for prefix in ('Заказы', 'Продукция'):
        for n, t, tt in cmds:
            if (n in STATUS or n in CREATE) and n not in NEW_CREATE:
                cid += 1
    ...
    # Команды редакции 5 - после прежних: номера прежних команд не меняются.
    for prefix in ('Выпуски', 'Отгрузки'):
        for n in PURCHASE_STATUS:
            action, title, tooltip = PURCHASE_CMDS[n]
            cid += 1
            fcmds += table_command(prefix + n, cid, title, tooltip, action, prefix)
    ...
    for n in PURCHASE_STATUS:
        action, title, tooltip = PURCHASE_CMDS[n]
        cid += 1
        fcmds += table_command('Переработки' + n, cid, title, tooltip, action, 'Переработки')
    cid += 1
